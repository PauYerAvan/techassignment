...
let setNumberOfColumns = (newValue: number) => {  
    if (newValue >=1 && newValue <=3) {
        return Grid.numberOfColumns = newValue
    }
    else return new Error('number incorrect!')
}
...
let getNumberOfBusySlots = () => {
    ...
    for(let i = 0; i < Grid.currentSlots.length; i++) {
        if (Grid.currentSlots[i].state === 'busy') count++
    }
    return count
};
...
let getNumberOfEmptySlots =  () => {
    ...
    for(let i = 0; i < Grid.currentSlots.length; i++) {
        if (Grid.currentSlots[i].state === 'empty') count++
    }
    return count
};
...
let onSlotSelected = (index:number) => {

    if (Grid.currentSlots[index].state === 'busy') return 'the slot is already busy'
    
    else if (getNumberOfEmptySlots() <= 2) return 'maximum number of busy slots completed'
...
let onSlotClosed = (index:number) => {

    if (Grid.currentSlots[index].state === 'empty') return 'the slot is already empty'
